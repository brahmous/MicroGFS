...
public:
      ...
      : server_info_{server_info},
        chunk_server_stub_{
            GFSChunkServer::ChunkServerService::NewStub(grpc::CreateChannel(
                grpc_connection_string<tcp_rpc_server_descriptor_t>(
                    server_info_),
  ...
  void AssignLease(const GFSNameSpace::write_coordinate &write_coordinates,
    ...
    request.set_handle(
        write_coordinates.chunk_server_handle_list.front().handle);
    ...
    for (int i = 1; i < write_coordinates.chunk_server_handle_list.size() - 1;
         ++i) {
      GFSChunkServer::SecondaryAndForwardServerInfo
          *secondary_and_forward_info =
  ...
  tcp_rpc_server_descriptor_t server_info() { return server_info_; }
  ...
  public:
    ...
    CreateFile(grpc::CallbackServerContext *context,
      ...
      if (valid_file_description == true) {
        if (auto find_result =
                master->file_name_to_integer_index.find(file_path);
    ...
    Write(grpc::CallbackServerContext *context,
      ...
      if (found) {
        ...
        if (file.size == 0) {
          ...
          std::make_heap(
              master->chunk_server_ordering_.begin(),
          ...
          for (int i = 0; i < write_replica_count; ++i) {
            write_coordinates.chunk_server_handle_list.push_back(
                {master->handle_counter_++,
                 master->data_servers_[master->chunk_server_ordering_[i]]
                     .server_info_});
          }
          master->data_servers_[*master->chunk_server_ordering_.begin()]
              .AssignLease(write_coordinates, acknowledged);
        } else {
          ...
          if (chunk_index >= file.chunks.size()) {
            ...
            for (const GFSNameSpace::chunk_replica_descriptor
                     &replica_descriptor : file.chunks[chunk_index].replicas) {
              write_coordinates.chunk_server_handle_list.push_back(
                  {replica_descriptor.handle,
                   master->data_servers_[replica_descriptor.host_id]
                       .server_info_});
            ...
            master
                ->data_servers_
                    [host_ip_id_map[write_coordinates.chunk_server_handle_list
                                        .begin()
                                        ->server_info.ip]]
                .AssignLease(write_coordinates, acknowledged);
          }
      ...
      if (acknowledged) {
        response->mutable_response_body()->set_write_id(
            write_coordinates.write_id);
        ...
        response->mutable_response_body()
            ->mutable_primary_server()
            ->set_tcp_port(primary.tcp_port);
        response->mutable_response_body()
            ->mutable_primary_server()
            ->set_rpc_port(primary.rpc_port);
        for (int i = 1; i < write_replica_count; ++i) {
          auto response_secondary_server = response->mutable_response_body()
                                               ->mutable_secondary_servers()
                                               ->Add();
...
private:
  ...
  public:
    ...
    bool operator()(int a, int b) {
      return (*(this->data_servers_))[a].priority <
             (*(this->data_servers_))[b].priority;
    }
